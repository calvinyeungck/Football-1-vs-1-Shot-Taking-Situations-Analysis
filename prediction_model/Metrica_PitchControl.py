...
#The following code has been edited to fit the requirements of the research
...
def check_offsides( attacking_players, defending_players, ball_position, GK_numbers, verbose=False, tol=0.2):
    ...
    defending_half=1 #attcking is always from left to right in the data
...
class player(object):
    ...
    def get_position(self,team):
        ...
        self.position = np.array( [ team['x'], team['y'] ] ) #adjust for different data format
        
    def get_velocity(self,team):
        # self.velocity = np.array( [ team[self.playername+'vx'], team[self.playername+'vy'] ] )
        # if np.any( np.isnan(self.velocity) ):
        #     self.velocity = np.array([0.,0.])
        self.velocity = np.array([0.,0.]) #set to zero since we don't have velocity data
...
def default_model_params(time_to_control_veto=3):
    ...
    params['int_dt'] = 0.01 # integration timestep (dt)
...
def calculate_pitch_control_at_target(target_position, attacking_players, defending_players, ball_start_pos, params):
    ...
    if tau_min_att-max(ball_travel_time,tau_min_def) >= params['time_to_control_def']:
        ...
        # attacking_players = [p for p in attacking_players if p.time_to_intercept-tau_min_att < params['time_to_control_att'] ] #not removing any attacking player since only the offball attacking player is being considered
        defending_players = [p for p in defending_players if p.time_to_intercept-tau_min_def < params['time_to_control_def'] ]
        # set up integration arrays (time 0 to the travel time of the ball to the target location)
        # dT_array = np.arange(ball_travel_time-params['int_dt'],ball_travel_time+params['max_int_time'],params['int_dt']) 
        dT_array = np.arange(0,ball_travel_time+params['int_dt'],params['int_dt']) 
